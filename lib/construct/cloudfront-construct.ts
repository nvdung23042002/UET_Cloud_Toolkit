...
interface CloudFrontConstructProps extends cdk.StackProps {
  ...
  /**
   * Application Load Balancer will be used for origin
   */
  appAlbs: elbv2.ApplicationLoadBalancer[];
  /**
   * preSharedKey value for communication with ALB
   * Require if integrated with ALB
   *
   * @default - undefined will not create custom response header
   */
  preSharedKey?: string;
}
...
export class CloudFrontConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: CloudFrontConstructProps) {
    ...
    const cfCertificateArn = `arn:aws:acm:us-east-1:${cdk.Stack.of(this).account}:certificate/${props.CertificateIdentifier.identifier}`;
      ...
      const cfDistribution = new cloudfront.Distribution(this, 'Distribution', {
        defaultBehavior: {
          origin: new origins.LoadBalancerV2Origin(props.appAlbs[0], {
            ...
            customHeaders: props.preSharedKey ? {
              'x-pre-shared-key': props.preSharedKey,
            } : undefined,
          }),
          viewerProtocolPolicy: cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS,

...
def login_ecr(aws_account_id):
    ...
    try:
        login_command = "aws ecr get-login-password --region ap-southeast-1 | docker login --username AWS --password-stdin 445201560187.dkr.ecr.ap-southeast-1.amazonaws.com"
...
def update_conf_bucket(conf_master_bucket, conf_bucket, zip_name):
    ...
    if not file_exists:
        print(
            "image_backend.zip does not exist in ConfBucket, so get it from ConfMasterBucket"
        )
        s3.copy_object(
        ...
        print("image_backend.zip already exists in ConfBucket, so relocate image.zip")
        tmp_file = "image_backend_tmp.zip"
...
def main():
    ...
    aws_account_id = "44520156018"
    print(f"AWS AccountID is {aws_account_id}")

    # ECRのARN取得
    print("Setting ECR ARN...")
    image_repo_name = "devblea-ecs-devbleaecsappdevbleaecsbackend2backappecsresourcesrepo3aeeb4c6-hoqq9eaj8ehl"
